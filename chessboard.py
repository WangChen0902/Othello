...
class ChessBoard(object):
    def __init__(self):
        self.__lineboard = [EMPTY for n in range(64)]
        self.__board = [[EMPTY for n in range(8)] for m in range(8)]
    ...
    def lineboard(self):
        for i in range(0,8):
            for j in range(0,8):
                self.__lineboard[i * 8 + j] = self.__board[i][j]
        return self.__lineboard
